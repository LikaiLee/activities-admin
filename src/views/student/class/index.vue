<template>
  <div class="school">
    ...
    <student-table type="class" @update="showUpdateDialog" :data="list" :loading="loading" :fromIndex="fromIndex" />
    <simple-pagination @pageChanged="handlePageChanged" :show="!loading" :data="list" :pageSize="pageSize" />
    <update-stu-info-dialog type="class" :user="temp" @confirm="handleUpdate" @cancel="toggleDialog" @visibleChange="handleVisibleChange" :visible="visible" />
  </div>
</template>
...
import { fetchStudentByPage, fetchStudentByStuId, updateStudent } from '@/api/student/classAdmin'
export default {
  data() {
    return {
      list: [],
  ...
  methods: {
    ...
    handleUpdate({ stuId, political }) {
      this.toggleDialog()
      updateStudent({ stuId, political }).then((res) => {
        this.$message({
          message: res.message,
          type: 'success'
        })
