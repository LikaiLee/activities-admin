<template>
  <div>
    <upload-excel :header="header" @on-selected-file='handleSelected'>
      ...
      <a href="http://120.25.240.194:8080/stu/db/reserve/file" ref="anchor" style="display: none;"></a>
    </upload-excel>
    ...
    <el-table :data="list" border highlight-current-row style="width: 100%;margin-top:20px;">
      ...
      <el-table-column prop="cause" label="加分原因" align="center" />
...
import { uploadReserveFile } from '@/api/db'
import { stuReserveTplFields } from '@/config'
export default {
  data() {
    return {
      header: ['stuId', 'stuName', 'cause', 'score', 'common'],
      file: null,
      list: [],
      pageSize: 10,
      currentPage: 1,
  ...
  methods: {
    ...
    handleUpload() {
      ...
      this.$confirm('即将上传文件, 是否继续?', '提示', {
        ...
        uploadReserveFile(this.file).then(({ data, message }) => {
          if (data.length) {
    ...
    validateTableHeader() {
      ...
      for (let i = 0; i < tableHeader.length; i++) {
        if (tableHeader[i] !== stuReserveTplFields[i]) {
          flag = false
          return flag
        }
      }
