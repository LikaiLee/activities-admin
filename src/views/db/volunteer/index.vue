<template>
  <div>
    <upload-excel :header="header" @on-selected-file='handleSelected'>
      ...
      <a href="http://120.25.240.194:8080/stu/db/volunteer/file" ref="anchor" style="display: none;"></a>
    </upload-excel>
    ...
    <el-table :data="list" border highlight-current-row style="width: 100%;margin-top:20px;">
      ...
      <el-table-column prop="volunteerName" label="志愿名称" align="center" />
      <el-table-column prop="volunteerDate" label="志愿时间" align="center" />
      <el-table-column prop="volunteerTime" label="志愿时长" align="center" />
...
import { uploadVolunteerFile } from '@/api/db'
import { stuVolunteerTplFields } from '@/config'
export default {
  data() {
    return {
      header: ['stuId', 'stuName', 'volunteerName', 'volunteerDate', 'volunteerTime', 'common'],
      file: null,
      list: [],
      pageSize: 10,
      currentPage: 1,
  ...
  methods: {
    ...
    handleUpload() {
      ...
      this.$confirm('即将上传文件, 是否继续?', '提示', {
        ...
        uploadVolunteerFile(this.file).then(({ data, message }) => {
          if (data.length) {
    ...
    validateTableHeader() {
      ...
      for (let i = 0; i < tableHeader.length; i++) {
        if (tableHeader[i] !== stuVolunteerTplFields[i]) {
          flag = false
          return flag
        }
      }
