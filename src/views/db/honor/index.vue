<template>
  <div>
    <upload-excel :header="header" @on-selected-file='handleSelected'>
      ...
      <a href="http://120.25.240.194:8080/stu/db/honor/file" ref="anchor" style="display: none;"></a>
    </upload-excel>
    ...
    <el-table :data="list" border highlight-current-row style="width: 100%;margin-top:20px;">
      ...
      <el-table-column prop="honorLv" label="等级" align="center" />
      <el-table-column prop="honorScore" label="加分" align="center" />
...
import { uploadHonorFile } from '@/api/db'
import { stuHonorTplFields } from '@/config'
export default {
  data() {
    return {
      header: ['stuId', 'stuName', 'honorName', 'honorLv', 'honorScore', 'common'],
      file: null,
      list: [],
      pageSize: 10,
      currentPage: 1,
  ...
  methods: {
    ...
    handleUpload() {
      ...
      this.$confirm('即将上传文件, 是否继续?', '提示', {
        ...
        uploadHonorFile(this.file).then(({ data, message }) => {
          if (data.length) {
    ...
    validateTableHeader() {
      ...
      for (let i = 0; i < tableHeader.length; i++) {
        if (tableHeader[i] !== stuHonorTplFields[i]) {
          flag = false
          return flag
        }
      }
