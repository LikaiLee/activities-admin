<template>
  <div>
    ...
    <el-table :data="list" border highlight-current-row style="width: 100%;margin-top:20px;">
      ...
      <el-table-column prop="practiceName" label="实践名称" align="center" />
      <el-table-column prop="practiceDate" label="实践时间" align="center" />
      <el-table-column prop="result" label="实践结果" align="center" />
...
import { uploadPracticeFile } from '@/api/db'
import { stuPracticeTplFields } from '@/config'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    handleUpload() {
      ...
      this.$confirm('即将上传文件, 是否继续?', '提示', {
        ...
        uploadPracticeFile(this.file).then(({ data, message }) => {
          if (data.length) {
    ...
    validateTableHeader() {
      ...
      for (let i = 0; i < tableHeader.length; i++) {
        if (tableHeader[i] !== stuPracticeTplFields[i]) {
          flag = false
          return flag
        }
      }
